...
$query = mysql_query("SELECT name_Event FROM `event`")or die(mysql_error());
...
<div>
  ...
  <tr>
   ...
   <input type="submit" value="إضافة" name="save" data-toggle="tooltip" data-placement="bottom"  class="btn btn-primary" >
	      ...
	      <?php
    if(isset($_POST['save'])){
        $eventName = $_POST["addSubEvent"];
        $query1 = mysql_query("SELECT event_ID FROM `event` WHERE name_Event = '$eventName'")or die(mysql_error());
        $row1 = mysql_fetch_array($query1);
        $eventID =$row1['event_ID'];
        $subName = $_POST["Sname"];
        $disSub = $_POST["subDescription"];
        $sql = mysql_query( "INSERT INTO subevent ( subevent_ID, event_ID, nameSubEvent ,description_subevent) VALUES ('','$eventID','$subName','$disSub')")or die(mysql_error());
        if($sql!=null)
